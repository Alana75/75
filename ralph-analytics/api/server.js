...
app.get('/client/field-audit',  (_req, res) => res.sendFile(path.join(STATIC,'client-portal/field-audit.html')));
...
const CORE_PKGS = { rcars:3040, scvms:3050, irmp:3060, iarfs:3070, capts:3080, dashboard:3090, reporting:3100 };
...
app.get('/api/rba/training/data', requireAuth, (req, res) => {
  ...
  res.json(trainingStore[key] || {});
});

app.post('/api/rba/training/maturity', requireAuth, (req, res) => {
  const key = req.user?.id || 'default';
  if (!trainingStore[key]) trainingStore[key] = {};
...
app.post('/api/rba/training/completion', requireAuth, (req, res) => {
  const key = req.user?.id || 'default';
  if (!trainingStore[key]) trainingStore[key] = {};
...
app.post('/api/rba/training/kpis', requireAuth, (req, res) => {
  const key = req.user?.id || 'default';
  if (!trainingStore[key]) trainingStore[key] = {};
